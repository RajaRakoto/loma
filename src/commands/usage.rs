...
pub fn runUsage() -> crate::Result<()> {
    ...
    let report_types = vec!["RTK Gains", "CCUsage"];
    ...
    match choice {
        "RTK Gains" => {
            if lomaFs::cmdExists("rtk") {
                ...
                display::info(
                    "To install RTK globally, you can run the following installation script:",
                );
                ...
                if install {
                    ...
                    if status.success() {
                        ...
                        if lomaFs::cmdExists("rtk") {
                            ...
                            display::info(
                                "You can execute 'rtk gain' after restarting your shell session.",
                            );
                        }
                    } else {
        ...
        "CCUsage" => {
            ...
            let sources = vec![
                "Claude", "Codex", "OpenCode", "Amp", "Droid", "Codebuff", "Hermes", "Goose",
                "OpenClaw", "Kilo", "Kimi", "Qwen", "Copilot", "Gemini",
            ];
            ...
            let periods = vec!["Daily", "Weekly", "Monthly", "Session", "Blocks"];
            let period_choice = Select::new("Step 2 — Sélection de la période:", periods)
                .prompt()
                .map_err(|e| crate::Error::other(e.to_string()))?;

            // Step 3: Select format
            let formats = vec!["Standard", "Compact"];
            let format_choice = Select::new("Step 3 — Sélection du format:", formats)
                .prompt()
                .map_err(|e| crate::Error::other(e.to_string()))?;

            // Step 4: Select scope
            let scopes = vec!["Projet courant (répertoire où Loma a été lancé)", "Global"];
            ...
            display::step(&format!(
                "Génération et exécution automatique de la commande: {}",
                full_command_str
            ));
...
pub fn buildCcusageArgs(
    _runner_cmd: &str,
    has_ccusage_arg: bool,
    source_choice: &str,
    period_choice: &str,
    format_choice: &str,
    scope_choice: &str,
...
mod tests {
    ...
    #[test]
    fn test_build_ccusage_args_standard_global() {
        let args = buildCcusageArgs(
            "ccusage",
            false,
            "Claude",
        ...
        assert_eq!(args, vec!["claude".to_string(), "daily".to_string()]);
    }

    #[test]
    fn test_build_ccusage_args_compact_project() {
        let args = buildCcusageArgs(
            "npx",
            true,
            "Gemini",
